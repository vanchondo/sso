...
public class LoginHandler {
        ...
        .flatMap(
            dto ->
                captchaValidatorService
                    .validateCaptcha(dto, request.exchange())
  ...
  public Mono<ServerResponse> handleCurrentUser(ServerRequest request) {
    ...
    log.info("{}Entering method", methodName);
    return request
        .attribute(Constants.CURRENT_USER_ATTRIBUTE)
  ...
  public Mono<ServerResponse> handleDeleteUser(ServerRequest request) {
    ...
    log.info("{}Entering method", methodName);
    return request
        .bodyToMono(DeleteUserDTO.class)
        ...
        .flatMap(
            user -> {
              CurrentUserDTO currentUser =
                  (CurrentUserDTO) request.attribute(Constants.CURRENT_USER_ATTRIBUTE).orElse(null);
              if (currentUser == null) {
  ...
  public Mono<ServerResponse> handleProfilePicture(ServerRequest request) {
    ...
    return request
        ...
        .flatMap(
            profilePicture -> {
              String mimeType =
                  Optional.ofNullable(profilePicture.getType()).orElse(MediaType.IMAGE_JPEG_VALUE);
              byte[] picture = profilePicture.getPicture();
              return ArrayUtils.isEmpty(picture)
                  ? ServerResponse.noContent().build()
                  : ServerResponse.ok().contentType(MediaType.valueOf(mimeType)).bodyValue(picture);
            });
  }
}
