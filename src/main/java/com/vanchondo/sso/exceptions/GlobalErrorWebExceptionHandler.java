...
public class GlobalErrorWebExceptionHandler extends AbstractErrorWebExceptionHandler {
  ...
  private static final String logMessage =
      "::handle:: Handle exception response for errorMessage={}";
  ...
  public static Mono<ServerResponse> handle(
      ReCaptchaInvalidException ex, ServerWebExchange exchange) {
  ...
  private static Mono<ServerResponse> buildResponse(
    ...
    return buildResponse(
        status.getReasonPhrase(), status, Collections.singletonList(message), exchange);
  }

  private static Mono<ServerResponse> buildResponse(
      HttpStatus status, List<String> messages, ServerWebExchange exchange) {
    return buildResponse(status.getReasonPhrase(), status, messages, exchange);
  }

  private static Mono<ServerResponse> buildResponse(
      String error, HttpStatus status, List<String> messages, ServerWebExchange exchange) {
    return ServerResponse.status(status)
        .bodyValue(new ErrorDTO(error, status.value(), getRequestURI(exchange), messages));
  }
